...
for arq in ARQUITECTURAS:
    ...
    arq_scores[str(arq)] = scores.mean()
    print(f"  Arquitectura {str(arq):<15} → CV R²={scores.mean():.4f}")

mejor_arq_str = max(arq_scores, key=arq_scores.get)
mejor_arq     = ARQUITECTURAS[[str(a) for a in ARQUITECTURAS].index(mejor_arq_str)]
print(f"\n Mejor arquitectura: {mejor_arq} (CV R²={arq_scores[mejor_arq_str]:.4f})\n")

pipeline = Pipeline([
    ("imputer", SimpleImputer(strategy="median")),
    ("scaler",  StandardScaler()),
    ("model",   MLPRegressor(
        hidden_layer_sizes=mejor_arq,
        activation="relu",
        solver="adam",
        max_iter=1000,
        random_state=SEED,
...
print("Validación cruzada (mejor arquitectura):")
...
fig.suptitle(f"{NOMBRE} {mejor_arq} — Predicción de PER NBA\nIntroducción al Aprendizaje Automático · UTFSM 2026",
...
ax2 = axes[1]
ax2.plot(loss_curve, color=COLOR, linewidth=1.5)
ax2.set_xlabel("Iteración")
ax2.set_ylabel("Loss (MSE)")
ax2.set_title(f"Curva de Pérdida durante Entrenamiento\n({n_iters} iteraciones)")
